# dovetell sample

Public, cloneable Dovetell samples.

This repo contains public-safe snapshots of Dovetell starter patterns. Use it
when you want to inspect, fork, or adapt a sample without starting from a docs
page.

## Samples

| Sample | Start here | Purpose |
| --- | --- | --- |
| Minimal Repo Context Starter | [`minimal-repo-context-starter/`](minimal-repo-context-starter/) | Add a tiny repo-owned context loop for AI-assisted work. |

## Release Boundary

This repo contains reviewed, public-safe sample snapshots.

It is meant for cloning, forking, inspection, and lightweight adaptation. It is
not a place for private strategy, customer details, credentials, internal launch
notes, or unreconciled roadmap material.

See [`REVISIONS.md`](REVISIONS.md) for public sample revisions.

## How To Use

Clone the repo:

```sh
git clone https://github.com/dovetell-io/sample.git
```

Then copy the sample folder you need into your project and adapt it. Do not copy
sample language blindly; replace it with facts from your repo.
